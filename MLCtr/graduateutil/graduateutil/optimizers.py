...
class NesterovAcceleratedGradient():
    ...
    def update(self, w, grad_func):
        ...
        approx_future_grad = cupy.clip(grad_func(w - self.momentum * self.w_updt), -1, 1)
...
class Adagrad():
    ...
    def update(self, w, grad_wrt_w):
        ...
        return w - self.learning_rate * grad_wrt_w / cupy.sqrt(self.G + self.eps)
...
class Adadelta():
    ...
    def update(self, w, grad_wrt_w):
        ...
        self.E_grad = self.rho * self.E_grad + (1 - self.rho) * cupy.power(grad_wrt_w, 2)
        ...
        self.E_w_updt = self.rho * self.E_w_updt + (1 - self.rho) * cupy.power(self.w_updt, 2)
...
class RMSprop():
    ...
    def update(self, w, grad_wrt_w):
        # If not initialized
        if self.Eg is None:
            self.Eg = cupy.zeros(np.shape(grad_wrt_w))

        self.Eg = self.rho * self.Eg + (1 - self.rho) * cupy.power(grad_wrt_w, 2)

        # Divide the learning rate for a weight by a running average of the magnitudes of recent
        # gradients for that weight
        return w - self.learning_rate *  grad_wrt_w / cupy.sqrt(self.Eg + self.eps)
...
class Adam():
    ...
    def update(self, w, grad_wrt_w):
        # If not initialized
        if self.m is None:
            self.m = cupy.zeros(np.shape(grad_wrt_w))
            self.v = cupy.zeros(np.shape(grad_wrt_w))
        ...
        self.w_updt = self.learning_rate * m_hat / (cupy.sqrt(v_hat) + self.eps)
